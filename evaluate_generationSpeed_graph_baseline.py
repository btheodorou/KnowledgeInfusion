...
from ruleModels.graphModelBaseline import GraphModel
...
model.to(device)
checkpoint = torch.load('./save/graph_model_baseline', map_location=torch.device(device))
...
pickle.dump(secondsPerPatient, open('./results/generationSpeeds/graphBaselineSpeed.pkl', 'wb'))
pickle.dump(synthetic_ehr_dataset, open(f'./results/graphBaselineDataset.pkl', 'wb'))