...
from ruleModels.cccModel import CCNModel
...
start = time()
for i in tqdm(range(0, NUM_GENERATIONS, 4*config.ccn_batch_size)):
  bs = min([NUM_GENERATIONS-i, 4*config.ccn_batch_size])
...
pickle.dump(secondsPerPatient, open('./results/generationSpeeds/ccnSpeed.pkl', 'wb'))
pickle.dump(synthetic_ehr_dataset, open(f'./results/ccnDataset.pkl', 'wb'))