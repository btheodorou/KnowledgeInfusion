...
def sample_sequence(model, length, context, batch_size, device='cuda', sample=True):
  ...
  with torch.no_grad():
    for _ in range(length-1):
      prev = model.sample_plus(torch.cat((prev,empty), dim=1), sample)
...
pickle.dump(secondsPerPatient, open('./results/generationSpeeds/graphPlusSpeed.pkl', 'wb'))
pickle.dump(synthetic_ehr_dataset, open(f'./results/graphPlusDataset.pkl', 'wb'))