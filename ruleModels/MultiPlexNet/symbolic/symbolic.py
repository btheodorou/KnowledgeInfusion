...
class OrList(nn.Module):
    ...
    def sample(self, x, class_predictions):
        pred = self.all_predictions(x)
        log_py = class_predictions.log_softmax(dim=1)
        return pred[np.arange(len(log_py)), torch.multinomial(log_py.softmax(dim=1), 1).squeeze(-1)]